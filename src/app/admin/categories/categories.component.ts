...
export class CategoriesComponent implements OnInit {
  ...
  creatCategory() {
    if (this.edit_status) {
      this.commentsService
        .editCategory(this.categoryForn.value, this.actionID as string)
        .then(() => {
          this.getCategory();
        });
    } else {
      this.commentsService.addCategory(this.categoryForn.value).then(() => {
        this.getCategory();
      });
    }
  ...
  delCategory(index: ComponentsResponse) {
    ...
    deleteObject(task);
    this.commentsService.delCategory(index.id as string).then(() => {
      this.getCategory();
    });
  }
